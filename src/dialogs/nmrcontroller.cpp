    ...
    , m_dataStore(dataStore)
{
    ...
    NMR_CONTROLLER_LOG("Controller created with DataStore");
}

void NMRController::loadStructure(const QString& filename, const QString& name)
{
    NMR_CONTROLLER_LOG("Loading structure: " << filename);

    try {
        int index = m_dataStore->addStructure(filename, name);
        NMR_CONTROLLER_LOG("Structure loaded with index: " << index);
...
{
    NMR_CONTROLLER_LOG("Removing structure with index: " << index);
    m_dataStore->removeStructure(index);
}

void NMRController::clearAllStructures()
{
    NMR_CONTROLLER_LOG("Clearing all structures");
...
{
    NMR_CONTROLLER_LOG("Setting reference to index: " << index);
    m_dataStore->setReference(index);
}

void NMRController::setStructureVisible(int index, bool visible)
{
    NMR_CONTROLLER_LOG("Setting structure " << index << " visibility to: " << visible);
...
{
    NMR_CONTROLLER_LOG("Setting structure " << index << " scale factor to: " << factor);
...
{
    NMR_CONTROLLER_LOG("Setting compound " << compound << " scale factor to: " << factor);
...
{
    NMR_CONTROLLER_LOG("Setting element " << element << " visibility to: " << visible);
...
{
    NMR_CONTROLLER_LOG("Setting nucleus " << element << "_" << nucleusIndex << " visibility to: " << visible);
...
{
    NMR_CONTROLLER_LOG("Generating spectrum with " << plotPoints << " points and lineWidth " << lineWidth);
    ...
    NMR_CONTROLLER_LOG("Spectrum generation successful");
...
{
    NMR_CONTROLLER_LOG("Exporting data to file: " << filename);
    ...
    NMR_CONTROLLER_LOG("Data export successful");
...
{
    NMR_CONTROLLER_LOG("Sorting conformers for compound: " << compound);
...
{
    NMR_CONTROLLER_LOG("Data store changed notification received");
}
